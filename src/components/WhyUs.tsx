      ...
      <div className="mx-auto grid max-w-7xl items-center gap-14 px-5 lg:grid-cols-2 lg:gap-20 lg:px-8">
        <div>
          ...
          <p className="mt-4 text-lg leading-relaxed text-slate-600">
            Somos un estudio contable integral: acompañamos a pymes, profesionales y emprendedores en
            todo el ciclo de su empresa, desde la constitución hasta la planificación de su
            crecimiento.
          </p>

          <dl className="mt-10 space-y-8">
            {reasons.map((reason) =>
            <div key={reason.title} className="border-l-2 border-brand-orange pl-5">
                <dt className="text-base font-bold text-brand-navyDark">{reason.title}</dt>
                <dd className="mt-1.5 text-sm leading-relaxed text-slate-600">{reason.text}</dd>
              </div>
            )}
          </dl>
